...
class Department_check_model extends CI_Model {
  ...
  if($approval_officer != "NA")

     {
          ...
          $query = $this->db->get('employee');
           ...
           if( trim($approval_officer) == trim($this->session->emp_code) )   
           {
             ...
             redirect('apraisal_form/edit_department/'.base64_encode($emp_code).'');  
           }
            ...
            else if($forward_status == 1)
         {
           ...
           redirect('apraisal_form/view_department/'.base64_encode($emp_code).'');
    ...
    else if($forward_status == 0 && $dept_data == NULL)
     {
       
         if($fac_role == 2)
        {
          $this->load->view('v0.1/department_form_view_pri');
        }
        else
        {
        $this->load->view('v0.1/department_form_view');
         ...
         {
           redirect('apraisal_form/update_department/'.base64_encode($emp_code).'');
         }
