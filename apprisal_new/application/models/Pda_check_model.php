...
class Pda_check_model extends CI_Model {
     ...
     if($query->num_rows() > 0)
   {
    ...
    $query = $this->db->get('employee');
      ...
      if(trim($approval_officer) == trim($this->session->emp_code))   
     {
        ...
        redirect('apraisal_form/edit_pda/'.base64_encode($emp_code).'/');  
    ...
    if($forward_status == 0 && $pda_data == NULL)
     {
     ...
     else if($forward_status == 1 )
     {
        ...
        redirect('apraisal_form/view_pda/'.base64_encode($emp_code).'/');
    
     }

     else 
     {
       redirect('apraisal_form/update_pda/'.base64_encode($emp_code).'/');
     }
