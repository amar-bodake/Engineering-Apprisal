...
class Ahp_check_model extends CI_Model {
     ...
     if($query->num_rows() > 0)
   {
    ...
    $query = $this->db->get('employee');
      ...
      if(trim($approval_officer) == trim($this->session->emp_code))   
     {
       ...
       redirect('apraisal_form/edit_ahp/'.base64_encode($emp_code).'/');  
     ...
     else if($forward_status == 0 && $ahp_data == NULL)
     {
     ...
     {
       redirect('apraisal_form/update_ahp/'.base64_encode($emp_code).'/');
     }
