...
class Industry_check_model extends CI_Model {
  ...
  public function check() 
  {
     ...
     if($query->num_rows() > 0)
   {
...

    {
          ...
          $query = $this->db->get('employee');
           ...
           if( trim($approval_officer) == trim($this->session->emp_code) )   
           {
             ...
             redirect('apraisal_form/edit_industry/'.base64_encode($emp_code).'');  
           }
         ...
         else if($forward_status == 1)
         {
             ...
             redirect('apraisal_form/view_industry/'.base64_encode($emp_code).'');
    ...
    else if($forward_status == 0 && $insts_data == NULL)
     {
        if($role == 2)
        {
          $this->load->view('v0.1/industry_form_view_pri'); 
        }
        else if($role == 3)
        {
         ...
         {
           redirect('apraisal_form/update_industry/'.base64_encode($emp_code).'');
         }
